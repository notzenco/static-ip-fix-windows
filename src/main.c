...
static int run_cloudflare(void) {
  ...
  if (network_apply_doh(CF_DNS_IPV4_1, CF_DNS_IPV4_2, CF_DNS_IPV6_1,
                        CF_DNS_IPV6_2, CF_DOH_TEMPLATE) != 0) {
    network_rollback();
    return 1;
  }
...
static int run_google(void) {
  ...
  if (network_apply_doh(GOOGLE_DNS_IPV4_1, GOOGLE_DNS_IPV4_2, GOOGLE_DNS_IPV6_1,
                        GOOGLE_DNS_IPV6_2, GOOGLE_DOH_TEMPLATE) != 0) {
    network_rollback();
    return 1;
  }
